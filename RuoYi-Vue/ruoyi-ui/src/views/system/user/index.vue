<template>
      ...
      <el-col :span="20" :xs="24">
        <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
          ...
          <el-form-item label="职称" prop="title">
               ...
               <el-form-item label="参与排班" prop="skdFlag">
                       <el-select v-model="queryParams.skdFlag" placeholder="是否参与排班" clearable size="small">
                         <el-option
                           v-for="dict in skdFlagOptions"
                           :key="dict.dictValue"
                     ...
                     <el-form-item label="挂号级别" prop="registrationRankId">
                       <el-select v-model="queryParams.registrationRankId" placeholder="请选择挂号级别id" clearable size="small">
                         <el-option
                           v-for="dict in registrationRankIdOptions"
                           :key="dict.dictValue"
                           :label="dict.dictLabel"
                           :value="dict.dictValue"
                         />
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="科室" align="center" prop="dept.deptName" :show-overflow-tooltip="true" />
 ...
 <el-table-column label="职称" align="center" prop="title" />
      <el-table-column label="是否参与排班" align="center" prop="skdFlag" :formatter="skdFlagFormat" />
      <el-table-column label="挂号级别" align="center" prop="registrationRankId" :formatter="registrationRankIdFormat" />
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="归属科室" prop="deptId">
              <treeselect v-model="form.deptId" :options="deptOptions" :disable-branch-nodes="true" :show-count="true" placeholder="请选择归属科室" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
 ...
 <el-form-item label="职称" prop="title">
       ...
       <el-form-item label="参与排班">
                <el-select v-model="form.skdFlag" placeholder="是否参与排班">
                  <el-option
                    v-for="dict in skdFlagOptions"
                    :key="dict.dictValue"
                    :label="dict.dictLabel"
       ...
       <el-form-item label="挂号级别">
                <el-select v-model="form.registrationRankId" placeholder="请选择挂号级别">
                  <el-option
                    v-for="dict in registrationRankIdOptions"
                    :key="dict.dictValue"
                    :label="dict.dictLabel"
...
export default {
  ...
  created() {
    ...
    this.getDicts("sys_user_paiban").then(response => {
          this.skdFlagOptions = response.data;
        });
        this.getDicts("sys_user_level").then(response => {
          this.registrationRankIdOptions = response.data;
        });
  },
  methods: {

 // 是否参与排班：0-&gt;否;1-&gt;是字典翻译
    skdFlagFormat(row, column) {
      return this.selectDictLabel(this.skdFlagOptions, row.skdFlag);
    },
    // 挂号级别id字典翻译
    registrationRankIdFormat(row, column) {
      return this.selectDictLabel(this.registrationRankIdOptions, row.registrationRankId);
    },
    /** 查询用户列表 */
    getList() {
      this.loading = true;
